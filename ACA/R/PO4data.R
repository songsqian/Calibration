...
## Importing data
DUWC <- read_excel(paste(dataDIR, "Statistics for typical Ortho-P calibration_FINAL.xlsx", sep="/"), col_names=T, skip=5)[,1:5]
...
DUWC <- DUWC[tmp,]
## replacing test sample conc with NA
...
## exploring whether using log-log linear (not used in the paper)
...
## Monte Carlo simulation of linear regression models
## using function `posterior()` from package `rv`
### using rv::posterior
...
### MLE (Bayes fitting using MCMC in Stan, with and without samples)
LmMLE <- "
data {
...
"
## the log-log model (not used in the paper)
LmMLE_log <- "
data {
  int<lower=0> N; // sample size (training data, non-zero)
  int<lower=0> M; //observed ODs (for estimating concentration)
...
## prediction uncertainty is a function of how many test samples are included (not discussed in the paper).
...
## running the stan models for all tests one at a time without using BHM
LmMLE_stan_out <- list()
LmMLE_Cntrl <- list()
x_bars_mle <- numeric()
...
### BHM within (use centered x) (BHM within a test only)
LmBHM1 <- "
data {
  int<lower=0> N; // sample size (training data, non-zero)
  int<lower=0> M; //observed ODs (for estimating concentration)
